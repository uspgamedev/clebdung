...
func _process(_delta):
	if player_marker.position.x < 2 or map.texture.get_width() - 2 < player_marker.position.x \
	or player_marker.position.y < 2 or map.texture.get_height() - 2 < player_marker.position.y:
